...
**本机预览档案：11.5.3-beta2。** 该精确官方签名构建已完成桥接重建、可编辑草稿构建、
首页登记、原生播放、保存、完全退出、冷重开和结构回读；已另行复验叠化转场与轻微抖动。
原生 MP4 导出、既有工程编辑、蒙版、滤镜及其他原生效果仍保持禁用，不据此泛化到其他
Beta 或 11.5.3 build。
...
- 另有精确 `11.5.3-beta2` 草稿预览档案，仅适用于源码中记录的版本、build、程序库与工具链指纹。
...
- `11.5.3-beta2` 仅完成基础新建草稿、叠化转场与轻微抖动链路；原生导出、既有工程编辑、
  蒙版、滤镜及其他原生效果未验收并保持拒绝。
