...
| 系统 | macOS 26.0 或以上 |
| 剪映 | 国内版剪映专业版 11.5.0；11.4.2 有兼容配置；精确 11.5.3-beta2 仅有草稿预览档案；CapCut 国际版不适用 |
...
本机 `11.5.3-beta2` 预览档案只提供可编辑草稿构建、登记、保存和冷重开回读；
`tools/start_here.py build` 不会为该档案输出原生导出命令。不要把它当成正式版 11.5.0，
也不要手工扩大到其他 Beta build。
